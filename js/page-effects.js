      ...
      function init() {
        ...
        const updateFromPointer = function updateFromPointer(event) {
          ...
          updatePageScrollPosition(
            bounds.width > 0 ? ((event.clientX - bounds.left) / bounds.width) * 100 : 0,
          );
        };
        if (pageScrollTrack) {
          ...
          pageScrollTrack.addEventListener("pointermove", function handlePointermove(event) {
            if (pageScrollTrack.hasPointerCapture(event.pointerId)) updateFromPointer(event);
          });
          ...
          function handleScroll() {
            ...
            window.requestAnimationFrame(() => {
              ...
              if (pageProgressBar) {
                ...
                pageScrollTrack?.setAttribute(
                  "aria-valuetext",
                  clamped <= 0 ? "页面顶部" : clamped >= 100 ? "页面底部" : `页面 ${Math.round(clamped)}%`,
                );
              }
        ...
        if (canvas) {
          ...
          const particles = Array.from({ length: visualEffects.dustMaxParticles }, () => ({
            ...
            color:
              Math.random() < visualEffects.dustPrimaryRatio
                ? visualEffects.dustPrimaryColor
                : visualEffects.dustAccentColor,
          }));
          let frameId = 0;
          let lastTime = 0;
          let visible = true;
          const animateDust = (timestamp = 0) => {
            ...
            const frameFactor = lastTime
              ? Math.min(2, Math.max(0, (timestamp - lastTime) / visualEffects.dustFrameIntervalMs))
              : 1;
            lastTime = timestamp;
        ...
        if (heroVisualArea) {
          ...
          mascotFigure?.addEventListener("click", function handleClick() {
            if (!mascotSpeech) return;
            greetingIndex = (greetingIndex + 1) % greetings.length;
            const text = $("span", mascotSpeech);
            if (text) text.textContent = greetings[greetingIndex];
